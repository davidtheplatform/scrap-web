...
Blockdef* block_register(char* id) {
    // Block is temporary, so we don't return it
    Blockdef* block = vector_add_dst(&registered_blocks);
    block->id = id;
    block->inputs = vector_create();

    return &registered_blocks[vector_size(registered_blocks) - 1];
}

void block_add_text(Blockdef* block, char* text) {
    BlockInput* input = vector_add_dst(&block->inputs);
...
void block_unregister(Blockdef* block) {
    vector_free(block->inputs);
}

void draw_block(Vector2 position, char* text, Color color) {
    int text_size = conf.font_size * 0.6;
    int text_width = MeasureTextEx(font_cond, text, text_size, 0.0).x;
    Rectangle rect = {
        .x = position.x,
        .y = position.y,
        .width = text_width + 10,
    ...
    bool collision = CheckCollisionPointRec(GetMousePosition(), rect);
    Color final_color = collision ? ColorBrightness(color, 0.7) : color;
    float mult = collision ? 1.2 : 0.8;

    DrawRectangleRec(rect, final_color);
    DrawRectangleLinesEx(rect, 2.0, (Color){ MIN(final_color.r * mult, 255), MIN(final_color.g * mult, 255), MIN(final_color.b * mult, 255), final_color.a });
    DrawTextEx(font_cond, text, (Vector2){ rect.x + 5, rect.y + conf.font_size * 0.5 - text_size * 0.5 }, text_size, 0.0, collision ? BLACK : WHITE);
}
...
void draw_top_buttons(int sw, int sh) {
    ...
    DrawTextureEx(run_tex, run_pos, 0, (float)conf.font_size / 20.0f, WHITE);
}
...
void setup(void) {
    ...
    font = LoadFontEx("assets/nk57.otf", conf.font_size, codepoints, codepoints_count);
    font_cond = LoadFontEx("assets/nk57-cond.otf", conf.font_size, codepoints, codepoints_count);
    SetTextureFilter(font.texture, TEXTURE_FILTER_BILINEAR);
    SetTextureFilter(font_cond.texture, TEXTURE_FILTER_BILINEAR);

    registered_blocks = vector_create();

    Blockdef* some_block = block_register("some_block");
    block_add_text(some_block, "some block");
}

void free_registered_blocks(void) {
    for (int i = 0; i < vector_size(registered_blocks); i++) {
        block_unregister(&registered_blocks[i]);
    }
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginScissorMode(0, conf.font_size * 2, conf.side_bar_size, sh - conf.font_size * 2);
            ...
            draw_block((Vector2){ 10, conf.font_size * 2 + 10 }, "Basic block", (Color) { 0x00, 0x77, 0xff, 0xFF });
        EndScissorMode();
